...
class pssm_panel(wx.Panel):
    def __init__(self, parent, id):
        ...
        wx.StaticText(self, label="Explanation: Please choose between 1 and 8", pos = (330, 135))
        ...
        for eachText in [self.random_state,self.n_estimators]:
        ...
        #导入PSSM路径
        wx.StaticText(self, label="Concrete test sample :", pos = (770, 50))
        ...
        wx.StaticText(self, label="Predicted strength:", pos = (850, 195))
    ...
    def trainmodel(self):
        ...
        pressure = feature["混凝土抗压强度"]

        pressure_array = pressure

        feature.drop("混凝土抗压强度", axis=1, inplace=True)
        ...
        if classifier_selection_function==0:
            ...
            clf=RandomForestRegressor(n_estimators=int(self.n_estimators.GetValue()),random_state=int(self.random_state.GetValue()))
        elif classifier_selection_function==2:
            clf = AdaBoostRegressor()
        elif classifier_selection_function==3:
            clf =main.deepforest()
        else:
            clf = CatBoostRegressor(random_seed=0)
    ...
    def OnClick(self, event):
        if event.GetEventObject() == self.buttonimport1 :
            ...
            if classifier_selection_function==1:
                if self.n_estimators.GetValue()=='' or self.random_state.GetValue()=='':
                    dlg_4 = LoginDialog_4(None,-1)
                    dlg_4.ShowModal()
                    dlg_4.Destroy()
                    m=1
                elif self.n_estimators.GetValue().isdigit()==0 or self.random_state.GetValue().isdigit()==0:
...
class LoginDialog_3(wx.Dialog):
    def __init__(self, parent, id):
        ...
        self.sizer1 = wx.BoxSizer(wx.VERTICAL)
        self.sizer1.Add(wx.StaticText(self, -1, u"The number of features must be between 1 and 8"),
                         0, wx.ALIGN_CENTER|wx.TOP, border=50)
        self.sizer1.Add(wx.Button(self, wx.ID_OK), 0, wx.ALIGN_CENTER_HORIZONTAL|wx.TOP|wx.BOTTOM, border=10 )
        self.SetSizer(self.sizer1)
        self.Centre()
